...
// 【P1-4 拆分】两个表格的列定义与状态标签已迁至 ./repos/repoColumns（工厂函数注入 handler）
import { createRepoColumns, createMyRepoColumns, getStatusTag, type RepoStats } from './repos/repoColumns'

const { TextArea } = Input
const { TabPane } = Tabs
const { Text } = Typography

// 仓库统计信息接口 RepoStats 已迁至 ./repos/repoColumns（与列定义同源）
...
export default function AdminRepos() {
  ...
  // 【P1-4 拆分】列定义与状态标签已迁至 ./repos/repoColumns（工厂函数注入 handler）
  const columns = createRepoColumns({
    onApprove: (repo) => { setSelectedRepo(repo); setApproveModalVisible(true) },
    onReject: (repo) => { setSelectedRepo(repo); setRejectModalVisible(true) },
  ...
  const myRepoColumns = createMyRepoColumns({
    statsMap: myRepoStats,
    onEditEndpoints: () => navigate('/owner/repos'),
    onDelete: handleDelete,
    onViewDetail: handleViewMyRepoDetail,
  })
