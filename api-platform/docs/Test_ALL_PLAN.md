...
**当前结果**：`npm run test:unit` → **127 passed**
（13 个 spec：permissions 13 + client 22 + Layout 6 + ErrorContext 13 + useDevice 5 + Login 8 + Analytics 7 + chartData 5 + paymentErrors 16 + ApiTester 8 + ConsumptionDetails 8 + admin/Repos 8 + owner/Repos 8；其中 TC-FE-API-008 参数化展开为 10 条）。
...
```bash
cd d:/Work_Area/AI/API-Agent/api-platform/web
npm run test:unit        # 单次运行（当前 127 passed）
