...
**当前结果**：`npm run test:unit` → **210 passed**（23 个测试文件 = 22 个组件 spec + `tests/cases/frontend_cases.spec.ts` 用例库自检 7 条）。
...
```bash
cd d:/Work_Area/AI/API-Agent/api-platform/web
npm run test:unit        # 单次运行（当前 210 passed / 23 个测试文件 = 22 组件 spec + 用例库自检）
