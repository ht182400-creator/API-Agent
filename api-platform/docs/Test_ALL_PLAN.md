...
**当前结果**：`npm run test:unit` → **224 passed**（25 个测试文件 = 24 个组件 spec + `tests/cases/frontend_cases.spec.ts` 用例库自检 7 条）。
...
```bash
cd d:/Work_Area/AI/API-Agent/api-platform/web
npm run test:unit        # 单次运行（当前 224 passed / 25 个测试文件 = 24 组件 spec + 用例库自检）
