...
**当前结果**：`npm run test:unit` → **238 passed**（28 个测试文件 = 27 个组件 spec + `tests/cases/frontend_cases.spec.ts` 用例库自检 7 条）。
...
```bash
cd d:/Work_Area/AI/API-Agent/api-platform/web
npm run test:unit        # 单次运行（当前 238 passed / 28 个测试文件 = 27 组件 spec + 用例库自检）
