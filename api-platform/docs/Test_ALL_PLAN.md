...
**当前结果**：`npm run test:unit` → **135 passed**
（14 个 spec：permissions 13 + client 22 + Layout 6 + ErrorContext 13 + useDevice 5 + Login 8 + Analytics 7 + chartData 5 + paymentErrors 16 + ApiTester 8 + ConsumptionDetails 8 + admin/Repos 8 + owner/Repos 8 + Recharge 8；其中 TC-FE-API-008 参数化展开为 10 条）。
...
| TC-FE-RECHARGE-001~008 | 套餐/配置/余额并行加载并过滤未启用套餐 / **到账金额（固定赠送 + 比例赠送）** / 套餐下单参数 / 自定义金额上下限（UI 层 min/max）/ 自定义下单参数 / 两者互斥 / 失败不白屏 / 自定义到账计算 | `pages/developer/Recharge.tsx` | 组件 | ✅ |
...
```bash
cd d:/Work_Area/AI/API-Agent/api-platform/web
npm run test:unit        # 单次运行（当前 135 passed）
