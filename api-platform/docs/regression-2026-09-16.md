...
> ⚠️ **2026-09-17 修正**：本行结论需两处更正（当时未察觉）——
> ① **不是 100%**：Analytics spec 也贡献过（如 `TC-FE-ANA-008`），只是当时占比小；
> ② **归因口径有坑**：并行跑时按 vitest 的 `stderr` 归属标记做按文件统计会**串台**；
> 且同一 spec 的警告数会随"文件集与线程调度"变化**两个数量级**（同一文件单跑 75 条、
> 与另一 spec 一起跑仅 1 条）。→ **只能同口径前后对比，别读绝对值**。
> 详见 `docs/payment-flow-architecture.md` §7 / §7.5（含三层解法与 M4-lite 实测）。
