...
| M3 探测统一 + 删 ref | 🔶 **M3-a / M3-b₁ / M3-c₁ 已完成**（a：探测**策略**统一 —— `isPaidStatus`/`isTerminalStatus` 收敛 11 处散落判定，修掉 `api/payment.ts` 只认 `paid` 漏 `completed`；b₁：两套轮询**重复的时间表**收敛到状态机常量；c₁：「结算后延迟刷新」3 处重复收敛为 `useDelayedReload`，`useQrcodePolling` 不再接收状态快照，并治好"全量偶发红"）。**余下待做**：M3-b 合并两个 hook 为 `usePaymentProbe`（**必须同步改 `FIX-2`/`FIX-3` 并逐条复验**）；M3-c② 收编 4 类监听、删除 `paymentStateRef` | 见 test-log「轮次 M3-a / M3-c₁」 |
