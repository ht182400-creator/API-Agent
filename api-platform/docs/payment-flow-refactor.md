...
| M3 探测统一 + 删 ref | 🔶 **M3-a + M3-b₁ 已完成**（a：**探测策略统一** —— `isPaidStatus` / `isTerminalStatus` 收敛 11 处散落判定，并修掉 `api/payment.ts` 只认 `paid` 漏 `completed` 的漂移；b₁：两套轮询**重复的时间表**收敛到状态机常量 `QR_PROBE_INTERVALS` / `REDIRECT_PROBE_INTERVAL`）。**余下待做**：M3-b 把两个 hook 合并为一个 `usePaymentProbe`（**必须同步改 `FIX-2`/`FIX-3` 并逐条复验**，否则规则静默失效）；M3-c 收编 4 类监听并删除 `paymentStateRef` | 见 test-log「轮次 M3-a」（含 M3-b₁） |
