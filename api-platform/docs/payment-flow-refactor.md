...
| M3 探测统一 + 删 ref | ✅ **全部完成**（a：探测**策略**统一 —— `isPaidStatus`/`isTerminalStatus` 收敛 11 处散落判定，修掉 `api/payment.ts` 只认 `paid` 漏 `completed`；b₁：时间表收敛到状态机常量；c₁：「结算后延迟刷新」收敛为 `useDelayedReload`；c②：**`paymentStateRef` 快照已删除**；b：两套轮询合并为 **`usePaymentProbe` 统一调度器** —— start 时显式传 mode/paymentNo、探测动作按模式分流保留业务差异，`FIX-2`/`FIX-3` 随代码搬移并复验 13/13） | 见 test-log「轮次 M3-a / M3-c₁ / M3-c② / M3-b」 |
| M4 收编剩余 hook | ✅ **全部完成**（M4-lite：`useCountdown` 倒计时派生 + 关弹窗即停表，变异规则 `FIX-8/9`；M4：`usePaymentWindow` 支付窗口机械操作收编，`Recharge.tsx` 1539→**1419** 行） | 见 test-log「轮次 M4-lite / M4」 |
