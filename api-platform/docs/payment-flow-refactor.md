...
| M3 探测统一 + 删 ref | 🔶 **M3-a / M3-b₁ / M3-c₁ / M3-c② 已完成**（a：探测**策略**统一 —— `isPaidStatus`/`isTerminalStatus` 收敛 11 处散落判定，修掉 `api/payment.ts` 只认 `paid` 漏 `completed`；b₁：两套轮询**重复的时间表**收敛到状态机常量；c₁：「结算后延迟刷新」3 处重复收敛为 `useDelayedReload`，治好"全量偶发红"；c②：**`paymentStateRef` 状态快照已删除** —— focus/visibility 改为带依赖重挂、轮询条件改由页面注入 `shouldContinue()`）。**仅剩 M3-b**：合并两个轮询 hook 为 `usePaymentProbe`（**必须同步改 `FIX-2`/`FIX-3` 并逐条复验**） | 见 test-log「轮次 M3-a / M3-c₁ / M3-c②」 |
