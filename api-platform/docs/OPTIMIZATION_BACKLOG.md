...
#### 当前进度（2026-09-17 实测复核，行数一律以磁盘为准）

| 组件 | 拆分前 → 现在 | 抽出模块数 / 行数 | 状态 |
|------|--------------|------------------|------|
...
- `admin/analytics/`：`constants.ts`(45) · `chartData.ts`(50，带单测) · `repoDetailColumns.tsx`(119) ·
...
- `developer/recharge/`：`constants.tsx`(50) · `rechargeLogger.ts`(48) · `useRechargeData.ts`(77) ·
  `paymentSession.ts`(75) · **`useQrcodePolling.ts`(131)**（B3 轮）· **`usePaymentPolling.ts`(78)**（B4 轮）·
  `components/PackageCard.tsx`(84) · `components/PaymentSummary.tsx`(97) ·
  `components/PaySuccessView.tsx`(135) · **`components/PaymentModal.tsx`(188)**（B5 轮）
  ...
  > ⚠️ `usePaymentPolling` 同理交还 `paymentPollIntervalRef`（FIX-3 的 find 是"两行 ref + clearInterval"
  > 的组合）。**两个 hook 的调用点都有双重约束**：既要在其依赖的 const（`closePayWindow` /
  > `handleRefreshStatus`）**之后**（入参立即求值 → TDZ），又要在 `if (loading) return ...`
  > 早退分支**之前**（否则首屏提前 return → `Rendered fewer hooks than expected`，实测 18 条用例全红）。
...
### 3.1 P1-4 巨型文件拆分 🔄（后端 4/4 ✅；前端 3/4 已完成：`Analytics` · `admin/Repos` · `owner/Repos`，仅剩 `Recharge`（2001→1528，行为重构后剩余为页面骨架、收益递减）；另 `AdminLogs` 904→694 已拆（2026-09-17）｜详见 §2.25）
...
| `web/src/pages/developer/Recharge.tsx` | 🔶 **可独立部分已抽尽**：2001 → **1528** 行（§2.25；另见行为重构） | 已抽 2 个纯逻辑模块（`constants`/`rechargeLogger`）+ 4 个 hook（`useRechargeData`/`useQrcodePolling`/`usePaymentPolling`/`useDelayedReload` + `useCountdown`）+ 1 个会话模块（`paymentSession`）+ 4 个展示组件（`PackageCard`/`PaymentSummary`/`PaySuccessView`/`PaymentModal`）。**余下 1528 行是页面骨架 + 十余个事件处理器 + 状态编排，继续搬运式拆分收益递减** → 行为重构（支付流程收敛为状态机）已按另立议题完成（见下行） | 高 |
| **`Recharge` 支付流程行为重构** | ✅ **全部完成**（M1 状态机 / M2 接线 / M3 全部子步 / M4 全部子步 / M5 安全加固；`Recharge.tsx` 2001 → **1419** 行） | 方案见 [`docs/payment-flow-refactor.md`](payment-flow-refactor.md)（显式状态机 + 探测统一 + 派生值不落库；**不引入 XState/TanStack Query** 的理由见 §2.1；**与主流支付架构经验的对照**见 §2.5）；图文详解见 [`docs/payment-flow-architecture.md`](payment-flow-architecture.md)（架构 / 状态机 / 信号流转图 + 小白与专家双视角 + act 警告专题 §7）。M1：`paymentMachine.ts` + 21 条纯函数用例，锁住**终态不可逆 / 单结算入口 / 探测开关派生**等不变式；M2：`usePaymentFlow.ts` + **35 处生命周期写入全部收敛为 action**（`Recharge.tsx` 1571→1525 行，act 警告 102→88）；**M4-lite：剩余有效期改为「由 `expiresAt` 派生」+「关弹窗即停表」**（新增 `recharge/useCountdown.ts` + 3 条假定时器用例，变异规则 `FIX-8/9`，act 88→81；顺带修掉「自减式漂移」与「关弹窗后定时器空转」两个真实问题）；**M5：资金链路安全加固已落地** —— 四条"客户端可写信号直接结算"的路径（`storage` 事件 / `checkPaymentResult` 读 localStorage / `postMessage`×2）统一收敛为 `confirmPaymentWithServer()`（进 `confirming` → `getPaymentStatus` 求证 → 仅后端说 paid/completed 才结算），并修掉"挂载恢复时后端已说 paid 却停在等待支付"的反向缺陷（用例 022/023/024 + 变异规则 `FIX-10/11`）。⚠️ M5 过程中另发现一处**未改**的缺陷：`closePayWindow()` 在无支付窗口引用时会 `window.close()` 关掉**当前窗口**（被 storage/postMessage 路径调用）→ 见用例库 `FE-BUG-CLOSE-PAY-WINDOW-CLOSE-SELF`（**已修**：删除 `closePayWindow` 的自关窗口；关窗是支付返回页自己的职责，样板见 `PaymentSuccess.tsx`；回归锁 = 用例 022 的 `window.close` 反断言 + 变异规则 `FIX-12`）。**M3-a（探测策略统一）已完成**：`isPaidStatus` / `isTerminalStatus` 收敛 11 处散落的"是否已支付 / 是否终态"判定（含 `api/payment.ts` 只认 `paid` 漏 `completed` 的漂移修复），新增 `src/utils/paymentStatus.ts` + 3 条纯函数用例；M3-b（合并两套轮询为 `usePaymentProbe`，**须同步改 `FIX-2/3`**）待做；M3-b₁（轮询时间表收敛到状态机常量 `QR_PROBE_INTERVALS`/`REDIRECT_PROBE_INTERVAL`）、M3-c₁（「结算后延迟刷新」收敛为 `useDelayedReload`）与 M3-c②（**`paymentStateRef` 状态快照已删除**：focus/visibility 监听带依赖重挂、轮询条件由页面注入 `shouldContinue`）均已完成 | 高 |
...
拆分后跑全量测试（后端 269 条 `pytest`；前端 **292 条** `npm run test:unit`）
+ 前端构建 + 手工回归对应页面。
