...
| **`Recharge` 支付流程行为重构** | 🔄 **M1 / M2 / M4-lite 已完成**（M3 / M4 余项 / M5 待做） | 方案见 [`docs/payment-flow-refactor.md`](payment-flow-refactor.md)（显式状态机 + 探测统一 + 派生值不落库；**不引入 XState/TanStack Query** 的理由见 §2.1；**与主流支付架构经验的对照**见 §2.5）；图文详解见 [`docs/payment-flow-architecture.md`](payment-flow-architecture.md)（架构 / 状态机 / 信号流转图 + 小白与专家双视角 + act 警告专题 §7）。M1：`paymentMachine.ts` + 21 条纯函数用例，锁住**终态不可逆 / 单结算入口 / 探测开关派生**等不变式；M2：`usePaymentFlow.ts` + **35 处生命周期写入全部收敛为 action**（`Recharge.tsx` 1571→1525 行，act 警告 102→88）；**M4-lite：剩余有效期改为「由 `expiresAt` 派生」+「关弹窗即停表」**（新增 `recharge/useCountdown.ts` + 3 条假定时器用例，变异规则 `FIX-8/9`，act 88→81；顺带修掉「自减式漂移」与「关弹窗后定时器空转」两个真实问题）；**M5：资金链路安全加固已落地** —— 四条"客户端可写信号直接结算"的路径（`storage` 事件 / `checkPaymentResult` 读 localStorage / `postMessage`×2）统一收敛为 `confirmPaymentWithServer()`（进 `confirming` → `getPaymentStatus` 求证 → 仅后端说 paid/completed 才结算），并修掉"挂载恢复时后端已说 paid 却停在等待支付"的反向缺陷（用例 022/023/024 + 变异规则 `FIX-10/11`）。⚠️ M5 过程中另发现一处**未改**的缺陷：`closePayWindow()` 在无支付窗口引用时会 `window.close()` 关掉**当前窗口**（被 storage/postMessage 路径调用）→ 见用例库 `FE-BUG-CLOSE-PAY-WINDOW-CLOSE-SELF`（需产品确认交互动线后再动） | 高 |
