...
> **📌 2026-09-15 更新**：本节方案已工程化为统一工具 `src/utils/time_range.py`，
> 并修正了两个局限：`<= 23:59:59` 闭区间丢亚秒数据（改半开区间）、手工 `timedelta` 散落（改统一函数）。
> 完整论证、业界对比与收敛记录见 **`docs/TIMEZONE_DESIGN.md`**。
> 新代码请使用 `cst_day_range_utc` / `cst_date_expr` / `cst_day_start_utc`，勿再手写时区换算。
